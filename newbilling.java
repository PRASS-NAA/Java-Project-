...
{
    JPanel panel;
    JLabel logo,total,paidamt,balamt,calculation,pdetails,pid,pname,qty,pprice,date,time,batchid,cashiername,cashierid;
    ...
    /*public static void generatePDF(Object[][] data) {
        try {
            ...
            // Create a table with the same number of columns as your JTable
            PdfPTable table = new PdfPTable(data[0].length);

            // Add column headers
            for (Object column : data[0]) {
                PdfPCell cell = new PdfPCell(new Phrase(column.toString()));
                table.addCell(cell);
            }
            ...
            for (int i = 1; i < data.length; i++) {
                for (int j = 0; j < data[i].length; j++) {
                    PdfPCell cell = new PdfPCell(new Phrase(data[i][j].toString()));
                    table.addCell(cell);
                }
            }
